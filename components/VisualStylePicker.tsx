...
const VisualStylePicker: React.FC<VisualStylePickerProps> = ({ selected, onSelect, disabled }) => {
  ...
  return (
    <div className="space-y-4">
      <label className="block text-[10px] font-black text-[#94a3b8] uppercase tracking-widest mb-4">
        Estilo Visual
      </label>
      <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-2 gap-3">
        {styles.map((style) => (
          <button
            ...
            className={`py-3.5 px-6 rounded-2xl text-[12px] font-bold transition-all duration-200 border ${
              selected === style
                ? 'bg-[#6366f1] border-[#6366f1] text-white shadow-lg shadow-indigo-100'
                : 'bg-white border-[#f1f5f9] text-[#64748b] hover:border-[#6366f1]/30 hover:bg-indigo-50/30'
