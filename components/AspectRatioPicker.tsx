...
const AspectRatioPicker: React.FC<AspectRatioPickerProps> = ({ selected, onSelect, disabled }) => {
  const options: { value: AspectRatio; label: string; icon: React.ReactNode }[] = [
    ...
    { value: '4:3', label: 'Paisaje', icon: <RectangleHorizontal className="w-6 h-6" /> },
    { value: '3:4', label: 'Retrato', icon: <RectangleHorizontal className="w-6 h-6 rotate-90" /> },
    { value: '4:5', label: 'Social', icon: <Smartphone className="w-6 h-6 scale-90" /> },
  ];
