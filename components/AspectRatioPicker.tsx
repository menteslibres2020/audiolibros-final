    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
      {options.map((opt) => (
        <button
          ...
          className={`relative overflow-hidden flex flex-col items-center justify-center p-6 rounded-[2rem] border-2 transition-all duration-300 group ${
            selected === opt.value
              ? 'border-transparent bg-gradient-to-br from-[#a78bfa] to-[#8b5cf6] text-white shadow-xl shadow-purple-200 scale-105 z-10'
              : 'border-white bg-white text-[#94a3b8] hover:border-purple-100 hover:text-purple-600 shadow-sm hover:shadow-md'
        ...
        >
          ...
          <svg
            className={`w-6 h-6 mb-3 transition-all duration-500 ${
              selected === opt.value ? 'text-white scale-110 rotate-3' : 'text-purple-200 group-hover:text-purple-400 group-hover:scale-110'
            }`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          ...
          <span className={`text-[9px] mt-2 font-bold uppercase tracking-tighter transition-opacity ${selected === opt.value ? 'text-white/70' : 'text-gray-300'}`}>
            {opt.value}
          </span>
        </button>
      ))}
